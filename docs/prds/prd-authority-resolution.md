...
> **Provenance.** This feature implements the **Authority Resolution Framework (ARF)**, a governance framework described in a research paper, as a first-class Ontos feature. ARF's central measurement — the DNA-Coefficient — is explicitly *proposed and not yet validated* in that paper; this PRD is the basis of a **PoC**, and the PoC is expected to feed corrections back into this document. Treat the DNAco scoring composite (see [Further Notes](#further-notes)) as a design surface pending methodology validation, not a frozen spec.
...
- **Deliberately not tested**: the exact DNAco composite math beyond the diverging-fraction contract (it is expected to change under methodology review); no SQL/JSON-shape snapshots beyond the contract surface.
...
- **The DNAco composite is our net-new IP and a methodology dependency.** The research paper defines the DNA-Coefficient qualitatively (a "normalized composite of the three instruments," proposed and unvalidated) and ships no formula. The v1 diverging-fraction scoring is a defensible starting point; the PoC is expected to refine it against the framework's methodology, and this PRD will iterate accordingly. Nothing downstream should hard-code the composite beyond the diverging-fraction contract the tests pin.
