...
const DEFAULT_AFFIRMATIONS: AffirmationInput[] = AFFIRMATION_ROLES.map((role) => ({
  role, principal: '', principal_type: 'user', required: true,
}));

export default function CreateAuthorityRelationDialog({ open, onOpenChange, onCreated }: Props) {
  const { post } = useApi();
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    const { data, error } = await post<AuthorityRelation>('/api/authority/relations', payload);
    setSaving(false);
    if (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>New Authority Relation</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="text-sm font-semibold text-muted-foreground pt-2">Affirmations (N-functional gate)</div>
          {affirmations.map((a, idx) => (
            <div key={idx} className="grid grid-cols-[1fr_1.5fr_auto_auto] gap-2 items-center">
              <select
          ...
          <Button
            ...
            onClick={() => setAffirmations((prev) => [...prev, { role: 'business', principal: '', principal_type: 'user', required: true }])}
          >
            <Plus className="h-4 w-4 mr-1" /> Add affirmer
          </Button>
